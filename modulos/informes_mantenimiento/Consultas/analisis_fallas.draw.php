...
if( !empty($_REQUEST["Accion"]) ){
    ...
    switch ($_REQUEST["Accion"]) {
        ...
        case 4:// dibujo la disponibilidad de las maquinas
            ...
            if($fecha_inicial=='' and $fecha_final==''){
                print('<div class="alert alert-outline alert-icon alert-dark alert-dismissible fade show">
                                        <div class="alert--icon">
                                            <i class="fa fa-exclamation-triangle"></i>
                                        </div>
                                        <div class="alert-text">
                                            <strong>Error!</strong> Debes elegir un rango de fechas para poder ver este reporte!
                                        </div>
                                        
                                    </div>');
                exit();
            }
            $DiferenciaDias=$obCon->obtenerDiferenciaFechasDias($fecha_final, $fecha_inicial);
            
            $horas_disponibilidad_total=24*$DiferenciaDias;
            ...
            $sql = "SELECT COUNT(*) as Items  
                   FROM $tabla t1 $Condicion GROUP BY maquina_id ;";
            ...
            $sql="SELECT sum(t1.tiempo_parada) as total_parada,
                  ...
                  FROM $tabla t1 $Condicion GROUP BY maquina_id  ORDER BY total_parada DESC LIMIT $PuntoInicio,$Limit; ";
            
            $Consulta=$obCon->QueryExterno($sql, HOST, USER, PW, $db, "");
            
            $CondicionBase64= base64_encode(urlencode($Condicion));
            $link_exportar_excel="procesadores/informes_mantenimiento.process.php?Accion=5&horas_disponibilidad_total=$horas_disponibilidad_total&empresa_id=$empresa_id&c=$CondicionBase64";
            $css->div("", "box-body no-padding", "", "", "", "", "");
                $css->div("", "mailbox-controls", "", "", "", "", "");
                    ...
                    print('<table class="table table-hover table-striped">');
                        print('<thead>
                                    <tr>
                                        ...
                                        <th>Maquina</th>
                                        <th>Ubicacion</th>
                                        <th>Disponibilidad ideal (horas)</th>
                                        <th>Tiempo de parada (horas)</th>
                                        <th>Disponibilidad de la maquina (%)</th>
                        ...
                        print('<tbody>');
                            while($RegistrosTabla=$obCon->FetchAssoc($Consulta)){
                                ...
                                $disponibilidad=round((($horas_disponibilidad_total-$RegistrosTabla["total_parada"])/$horas_disponibilidad_total)*100,2);
                                print('<tr>');
                                    ...
                                    print("<td class='mailbox-subject text-flickr'>");
                                        ...
                                        print("<strong>".number_format($horas_disponibilidad_total)."</strong>");
                                    print("</td>");
                                    print("<td class='mailbox-subject text-flickr'>");
                                        
                                        print("<strong>".number_format($RegistrosTabla["total_parada"])."</strong>");
                                    print("</td>");
                                    print("<td class='mailbox-subject text-flickr'>");
                                        
                                        print("<strong>".($disponibilidad)."%</strong>");
                                    print("</td>");
